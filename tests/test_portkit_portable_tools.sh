...
PACKAGED="$(sed -n '1p' "$ROOT/_kit/portkit-revision.txt")"
...
cmp "$RUNTIME" "$ROOT/ports/hk/dist/bin/portkit"

bash "$ROOT/_kit/dist_port.sh" appmanager >/dev/null
[ ! -e "$ROOT/ports/appmanager/dist/jenny92-appmanager/bin/portkit" ]

echo "portable PortKit tool tests: PASS"
