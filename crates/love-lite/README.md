...
- Reads controllers directly through SDL2, with APP-local calibrated mappings
  ahead of the bundled controller database. Does not start gptokeyb. Keyboard
  input remains available. Unknown controllers open a raw-input calibration
  overlay; mapped-but-wrong controllers can be recalibrated from the home menu,
  F2, or by holding two raw buttons for three seconds.
- Uses true event-driven redraws: static pages redraw only on input or Lua
  dirty marks. Toast edges and indeterminate busy spinners request 60 FPS only
  for their visible lifetime; task progress wakes the host at poll rate.
- Selects the GPU renderer automatically. The GPU subset covers rectangles,
  lines, polygons, axis-aligned circles/ellipses and cached text images. If
  accelerated SDL2 is unavailable, or a frame uses an unsupported operation
  such as stencil or a rotated circle, that work falls back to the CPU renderer.
...
Calibration requires all 12 inputs: four directions, A/B/X/Y, Start, Select, L1/R1.
There is no skip. Only an existing saved APP mapping allows Start + A to cancel
recalibration without replacing that mapping. First-time calibration cannot cancel.
Mappings are selected from the local file first, then bundled DB; a controller
absent from both enters calibration, even if SDL has a built-in mapping for it.
Each step requires release; hats and centered axis directions are accepted for
the d-pad. It is not an analog-stick calibration tool. Mappings are persisted
only in `APP/state/controller-mappings/<GUID>.txt`; no system or PortMaster files
are changed. SDL must expose the controller's raw input for calibration to work.
The host uses the first available controller and probes again after disconnect.
Outside calibration, Start/Select do not trigger UI actions. Idle input is polled at most every 33 ms without
redrawing clean pages; the calibration overlay redraws while active.
