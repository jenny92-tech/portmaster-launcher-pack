...
图片、Data 记住原来的根目录。首页右栏按“卸载 → 直接删除 → 回收站 → 全选/全不选 →
残留清理 → Runtime 修复”排列，让回收站紧跟卸载，两个维护工具下移。
“直接删除”是默认关闭、不写入配置的临时复选项；用户必须当次手动勾选并再通过红色
不可恢复 Dialog，卸载才会绕过回收站。操作完成后它自动恢复关闭。独立回收站页
