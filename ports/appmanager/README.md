...
`state/runtime-cache/<source commit>/<runtime>/`；重新进入修复时，随包 curl 使用 `-C -`
从已有字节继续。代理不支持 Range 时会安全地重新下载该分片；只有
