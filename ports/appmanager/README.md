...
- **TrimUI 的 PortMaster 核心不在 Data/ports** —— 官方 MainUI 包把核心放在
  `Apps/PortMaster/PortMaster`，并由同级 `launch.sh`、`config.json`、`icon.png`
  启动。环境健康检查、备份、安装和回滚必须把这四处当成同一事务。
- **发布通道按设备区分** —— MiniLoong 使用含机型适配的
  `jenny92-tech/PortMaster-GUI`；TrimUI 与其他官方设备读取
  `PortsMaster/PortMaster-GUI` 的 `version.json.stable`，下载其中指向的精确 ZIP
  并核对 MD5。Runtime 元数据始终来自官方 PortMaster-New。
