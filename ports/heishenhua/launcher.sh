...
# Stage 2: unityloader + wsm.toml — 字段由 stage 1 的 launch_config.env sed 写入
#
# 仓库里这是个模板:共用逻辑来自 _kit/portmaster_common.sh + launcher_unity_common.sh。
# 部署到设备时 _kit/assemble.sh 把 KIT 块原地内联成单个自包含脚本。下面只保留
# heishenhua 独有的 stage-1(frt3 UI)和 wsm.toml 改写(分辨率/质量/难度/按键)。
...
if [ -f "$HSH_ENV" ]; then
  ...
  if [ "$HSH_WIDTH" = "auto" ]; then
    HSH_WIDTH="$DISPLAY_WIDTH"; HSH_HEIGHT="$DISPLAY_HEIGHT"
    echo "$LOG_PREFIX resolution auto -> ${HSH_WIDTH}x${HSH_HEIGHT}"
  fi
  case "$HSH_WIDTH$HSH_HEIGHT" in
    *[!0-9]*|"") echo "$LOG_PREFIX bad resolution in env, leaving wsm.toml as-is" ;;
    *)
      sed -i "s/^displayWidth=.*/displayWidth=${HSH_WIDTH}/" "$GAMEDIR/wsm.toml"
      sed -i "s/^displayHeight=.*/displayHeight=${HSH_HEIGHT}/" "$GAMEDIR/wsm.toml"
      ;;
  esac
  # 画面质量: launcher UI 的 384/480/720/0 直接写到 textureMaxDim。
  # (中档原 512,部分场景闪退 → 480。仍接受旧的 512 以防旧 env 残留。)
  case "$HSH_TEXMAX" in
    0|384|480|512|720) sed -i "s/^textureMaxDim *=.*/textureMaxDim = ${HSH_TEXMAX}/" "$GAMEDIR/wsm.toml" ;;
  esac
